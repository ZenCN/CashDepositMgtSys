(function() {
    ...
    angular
        .module('app.page')
        .controller('gives_sum_details_ctrl', gives_sum_details_ctrl);

    gives_sum_details_ctrl.$inject = ['$scope', 'gives_sum_details_svr'];

    function gives_sum_details_ctrl(vm, svr) {
        ...
        vm.export = function() {
            if (angular.isObject(vm.search.result.sum)) {
                ...
                window.open((window.app_path ? window.app_path : '') + 'generation_gives/exportsumdetails?page_index=' + vm.page.index + '&page_size=' + vm.page.size + '&agency_code=' +
