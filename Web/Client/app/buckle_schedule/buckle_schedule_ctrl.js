(function () {
    ...
    angular
        .module('app.page')
        .controller('buckle_schedule_ctrl', buckle_schedule_ctrl);

    buckle_schedule_ctrl.$inject = ['$scope', 'buckle_schedule_svr'];

    function buckle_schedule_ctrl(vm, svr) {
        ...
        vm.export = function () {
            if (angular.isArray(vm.search.result)) {
                window.open((window.app_path ? window.app_path : '') + 'generation_buckle/exportschedule?page_index=' + vm.page.index + '&page_size=' + vm.page.size + '&agency_code=' +
